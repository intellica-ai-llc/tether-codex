...
export interface AgentResponse {
  agentId: string; agentName: string; content: string; action?: 'act' | 'wait'
}
...
export class AgentOrchestrator {
  ...
  constructor(projectId: string, storage?: any) {
    this.memory = storage || new TetherMemoryStorage(projectId)
    this.agents = new Map(ALL_PERSONAS.map(p => [p.id, p]))
    this.stateManager = new AgentStateManager()
  }

  async processMessage(message: string, context: OrchestratorContext) {
    if (this.memory instanceof TetherMemoryStorage) await this.memory.initialize()
    ...
    const memoryBlock = memoryContext.length > 0
      ? `<memory-context>\n${memoryContext.map((m: any) => `[${m.type}] ${m.key}: ${m.insight}`).join('\n')}\n</memory-context>`
      : ''
    ...
    // ********** THE KEY FIX: Record this interaction **********
    await this.memory.append({
      ts: new Date().toISOString(),
      type: 'observation',
      key: `agent-${agent.id}-interaction`,
      insight: `Agent ${agent.id} responded to: "${message.slice(0, 80)}"`,
      confidence: 8,
      source: 'orchestrator',
      tags: ['interaction', agent.id]
    })
